...
TMP=$(mktemp -d); trap 'kill $MOCK 2>/dev/null; pkill -f "http.server 1[34]00[0-9]" 2>/dev/null; rm -rf "$TMP"' EXIT
mkdir -p "$TMP/bin"
...
exec python3 -m http.server "\$P" --bind 127.0.0.1 --directory "$TMP"
X
done
printf '#!/usr/bin/env bash\necho "fake claude BASE=$ANTHROPIC_BASE_URL MODEL=$ANTHROPIC_MODEL FAST=$ANTHROPIC_SMALL_FAST_MODEL args=$*"\n' >"$TMP/bin/claude"
chmod +x "$TMP/bin/"*
...
check "wizard: auto code = deepseek-v4-pro" "Slot code.*|→ deepseek-ai/deepseek-v4-pro-0813 gewählt" < "$TMP/wiz.txt"
...
grep -q "model: openai/deepseek-ai/deepseek-v4-pro-0813" "$TMP/home/litellm.yaml" && { echo "  ok   litellm.yaml"; ((PASS++)); } || { echo "  FAIL litellm.yaml"; ((FAIL++)); }
check "status: services up" "Proxy +● läuft" < <(timeout 20 "$N" status)
check "code: env passed to claude" "MODEL=nim-code FAST=nim-fast args=--version" < <(timeout 30 "$N" code --version)
...
check "find: laguna responds" "laguna-xs-2.1 +antwortet" < <(timeout 30 "$N" find laguna)
...
check "pick: manual slot code → laguna" "code → poolside/laguna-xs-2.1" < <(printf '1\nlaguna\n1\n\nq\n' | timeout 60 "$N")
check "pick: dead model refused" "nicht geändert" < <(printf '1\nkimi-k2\n1\n\n\nq\n' | timeout 60 "$N")
grep -q 'MODEL_CODE="poolside/laguna-xs-2.1"' "$TMP/home/config" && { echo "  ok   pick persisted"; ((PASS++)); } || { echo "  FAIL pick persisted"; ((FAIL++)); }

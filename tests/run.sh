...
TMP=$(mktemp -d); trap 'kill $MOCK $MOCK_GROQ $MOCK_CEREBRAS $MOCK_MISTRAL $UPD 2>/dev/null; pkill -f "[f]ake_server.py ($PP|$CP|$IPT)" 2>/dev/null; pkill -f "[f]ake_searxng.py $SP" 2>/dev/null; pkill -f "$TMP/home/web/server.py" 2>/dev/null; rm -rf "$TMP"' EXIT
...
PP=$(( 20000 + ($$ % 2000) * 10 )); CP=$(( PP + 1 )); MP=$(( PP + 2 )); UP=$(( PP + 3 )); IPT=$(( PP + 4 )); SP=$(( PP + 5 )); GP=$(( PP + 6 )); CB=$(( PP + 7 )); WP=$(( PP + 8 )); MS=$(( PP + 9 ))   # proxy, chat, mock API, update server, IDE, search, mock Groq, mock Cerebras, web UI, mock Mistral
...
export NIMCTL_POOL_BASE_GROQ="http://127.0.0.1:$GP/v1" NIMCTL_POOL_BASE_CEREBRAS="http://127.0.0.1:$CB/v1" NIMCTL_POOL_BASE_MISTRAL="http://127.0.0.1:$MS/v1" NIMCTL_WEB_PORT=$WP NIMCTL_SCAN_RPM=600
...
python3 "$HERE/mock_api.py" "$MS" mistral & MOCK_MISTRAL=$!

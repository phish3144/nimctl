...
check "start: proxy bound to loopback" "fake litellm on 127.0.0.1:$PP" < "$TMP/home/logs/litellm.log"
check "start: idempotent" "läuft bereits" < <(timeout 60 "$N" start)
check "logs: proxy log shown" "fake litellm on" < <(timeout 10 "$N" logs proxy)
check "logs: unknown log name" "ungültige Eingabe" < <(timeout 10 "$N" logs nope)
...
check "status: services up (nimctl, pid)" "Proxy +✓ läuft +:$PP +nimctl · pid [0-9]+" < <(timeout 20 "$N" status)
...
python3 "$TMP/fake_server.py" $PP & FOREIGN=$!; sleep 0.5
check "start: foreign process on port refused" "Port $PP ist belegt" < <(timeout 30 "$N" start)

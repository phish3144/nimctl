...
grep -A1 'model_name: nim-chat$' "$Y" | grep -q "model: custom_openai/moonshotai/kimi-k2-instruct-0905, api_base: http://127.0.0.1:$GP/v1, api_key: os.environ/GROQ_API_KEY, max_tokens: 16384, timeout: 120," && pass "litellm.yaml: rank 1 of chat is its own group at Groq with Groq's key" || { fail "litellm.yaml: rank 1 of chat"; grep -A1 'nim-chat$' "$Y"; }

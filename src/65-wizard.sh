...
wizard() { # wizard [--yes]
  ...
  printf "\n  %b\n" "$(t wz_next)"; info "$(tf wz_chat_hint "$CHAT_PORT")"; [[ "$IDE_ENABLED" == 1 ]] && declare -F ide_url >/dev/null && info "$(tf ide_url "$(ide_url)")"; [[ "$WEB_ENABLED" == 1 ]] && info "$(tf web_url "$WEB_PORT")"; printf '\n'
}

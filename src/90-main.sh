...
status_json() {
  ...
  for s in "${SLOTS[@]}"; do m=$(slot_model "$s"); probe_get "$m"
    sj=$(jq -n --argjson acc "$sj" --arg s "$s" --arg m "$m" --arg res "$PROBE_RES" --arg ms "$PROBE_MS" --arg t "$PROBE_T" --arg tools "$PROBE_TOOLS" --arg chain "$(slot_chain "$s")" \
      '$acc + {($s): {model: (if $m=="" then null else $m end), state: (if $res=="" then "unprobed" elif $res=="ok" then "ok" else "error" end), error: (if $res=="ok" or $res=="" then null else $res end), ms: ($ms|tonumber? // null), probed: ($t|tonumber? // null), tools: (if $tools=="" then null else $tools end), chain: ($chain | split(" ") | map(select(length>0)))}}')
  done
  ...
  jq -n --arg v "$VERSION" --arg ks "$KEY_STATE" --arg kt "$KEY_TIME" --arg kd "$(key_days_left)" --argjson up_p "$up_p" --arg by_p "$by_p" --arg pid_p "$pid_p" --arg pp "$PROXY_PORT" \
    ...
    --argjson up_s "$up_s" --arg by_s "$by_s" --arg pid_s "$pid_s" --arg sp "$SEARCH_PORT" --arg se "$SEARCH_ENABLED" --argjson pool "$(pool_json)" --argjson health "$(jq -c . "$NIM_DIR/health.json" 2>/dev/null || echo "{}")" \
    ...
    '{version:$v, key:{state:$ks, checked:($kt|tonumber? // null), expires_in_days:($kd|tonumber? // null)},
      ...
      pool:$pool, health:$health, slots:$slots, tools:$tools}'
}
